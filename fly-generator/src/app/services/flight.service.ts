...
export class FlightService {
  ...
  flightControlUrl:string="http://localhost:12345/api/flight";
  ...
  starthubConnection(){
    this._hubConnection = new HubConnectionBuilder().withUrl('http://localhost:12345/terminal').build();
    this._hubConnection
    .start()
    .then(() => 
    {
      ...
      this.messageService.alertMsgEmitter('alert-success','Connection started!');
    })
    .catch(err =>
       {
         console.log('Error while establishing connection :(');
         
         this.messageService.alertMsgEmitter('alert-danger','Error while establishing connection :(');
        });
  }
  
  sendFly():void{
    let fly=this.getRundomFly();
    this.http.post<Flight>(this.flightControlUrl,fly,httpOptions).subscribe();
  }
  ...
  tryConnect():void{
    this.connectTimer= setInterval(() => {
      this.messageService.alertMsgEmitter('alert-warning','Connection will be restarted');
      this.starthubConnection()
    }, 3000);
  }
  flyGeneratorStart(){
  ...
  getRundomFly():Flight{
    ...
    while (!flight.nameOfСhiefPilot) {
      flight.nameOfСhiefPilot=NAMESOFPILOTS[this.randomNumber(1,NAMESOFPILOTS.length)];
    }
    while (!flight.from) {
